...
class AppAgentCardInfo extends Base
{
    /**
     * @node 充值卡管理
	 * 
	...
	public function setup()
    {
        ...
        $app_data = Apps::where('status',1)->order('id','asc')->select();
		$card_type = CardType::where('status',1)->select();
		
		
		foreach($app_data as $key=>$val){			
            foreach($card_type as $k=>$v){
            	$where = ['user_id'=>$user_id,'app_id'=>$val['id'],'card_type_id'=>$v['id']];				
                $rest_db = AgentCard::where($where)->find();
				if(!$rest_db){
					AgentCard::create($where);
				}
		    }
		}
		$i = 1;
		foreach($app_data as $key=>$val){
			$app_data[$key]['active'] = 0;
			if($i == 1){
				$app_data[$key]['active'] = 1;
			}
			$app_data[$key]['agent_data'] = AgentCard::where(['user_id'=>$user_id,'app_id'=>$val['id']])->select();
			foreach($app_data[$key]['agent_data'] as $k=>$v){
				$app_data[$key]['agent_data'][$k]['card_db'] = CardType::get($v['card_type_id']);
			}
			$i =$i+1;
    ...
    public function inSetNum(){
    	if(Request::instance()->isPost()){
    		$inputs=input('post.');
			if($inputs['rem_num']){
				AgentCard::where('id',$inputs['action_id'])->setInc('rem_num',$inputs['rem_num']);
				$agent_card = AgentCard::get($inputs['action_id']);
				$this->success('添加成功','',$agent_card);
			}
			$this->error('数量错误');
    	}
        $this->error('方式错误');
