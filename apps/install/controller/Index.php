...
class Index extends Controller {
	...
	public function index() {
		...
		if (is_file(APP_PATH . 'database.php')) {
			...
			$this -> assign('reinst', '删除public/data/install.lock升级<br>删除config/database.php重安装');
	...
	public function step4($db = null, $cover = 0) {
		...
		if ($this -> request -> isPost()) {
			// 检测数据库配置
			if (!is_array($db) || empty($db['type']) || empty($db['hostname']) || empty($db['database']) || empty($db['username']) || empty($db['prefix'])) {
	...
	public function complete() {
		if (session('step') != 4 && session('step') != 3 && session('step') != 2) {
		...
		if (session('error')) {
		    ...
		    file_put_contents('./public/data/install.lock', 'lock');
			session('step', null);
			session('error', null);
			session('reinstall', null);
			return $this -> fetch();
